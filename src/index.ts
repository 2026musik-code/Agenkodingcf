...
app.delete('/api/github/file', async (c) => {
    try {
        ...
        // Need SHA to delete file. If not provided, fetch it first.
        let fileSha = sha;
        if (!fileSha) {
             ...
             if (getRes.ok) {
                 ...
                 return c.json({ error: 'File not found or unable to fetch SHA for deletion' }, 404);
             }
        }
...
app.post('/api/chat', async (c) => {
  try {
    ...
    if (config.aiProvider === 'gemini') {
        ...
        if (!response.ok) {
            return c.json({ error: `Gemini API Error: ${response.status}`, details: data }, response.status);
        }
